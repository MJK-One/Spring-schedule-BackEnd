...
public class ScheduleController {
    ...
    public ResponseEntity<List<ScheduleDto>> findAllSchedules(
            ...
            @RequestParam(required = false) Long findAuthor, // 선택적 작성자 ID 파라미터
            @RequestParam(defaultValue = "0") int page, // 페이지 번호
            @RequestParam(defaultValue = "5") int size) { // 페이지 크기
        // 요청된 조건에 따라 모든 스케줄을 조회
        return new ResponseEntity<>(scheduleService.findAllSchedules(findDate, findName, findAuthor, page, size), HttpStatus.OK);
    }
