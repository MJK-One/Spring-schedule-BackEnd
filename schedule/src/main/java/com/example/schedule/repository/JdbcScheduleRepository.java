...
public class JdbcScheduleRepository implements IScheduleRepository {
    ...
    public ScheduleDto saveSchedule(ScheduleEntity schedule) {
        ...
        // DTO 반환
        return new ScheduleDto(key.longValue(), schedule.getTask(), schedule.getName(), schedule.getPassword(), LocalDateTime.now(), LocalDateTime.now());
    }

    // 모든 스케줄 조회 메서드
    @Override
    public List<ScheduleDto> findAllSchedule(LocalDate findDate, String findName) {
        // 기본 SQL 쿼리
    ...
    public ScheduleEntity findScheduleById(Long id) {
        ...
        return jdbcTemplate.queryForObject("SELECT * FROM schedule WHERE id = ?", ScheduleRowMapperV2(), id);
    }
    ...
    // ScheduleDto를 위한 RowMapper
    private RowMapper<ScheduleDto> ScheduleRowMapper() {
        return (rs, rowNum) -> new ScheduleDto(
    ...
    // ScheduleEntity를 위한 RowMapper
    private RowMapper<ScheduleEntity> ScheduleRowMapperV2() {
        return (rs, rowNum) -> new ScheduleEntity(
                rs.getLong("id"), // 결과 집합에서 "id" 컬럼 값을 Long으로 가져옴
                rs.getString("task"), // 결과 집합에서 "task" 컬럼 값을 String으로 가져옴
                rs.getString("name"), // 결과 집합에서 "name" 컬럼 값을 String으로 가져옴
                rs.getString("password"), // 결과 집합에서 "password" 컬럼 값을 String으로 가져옴
